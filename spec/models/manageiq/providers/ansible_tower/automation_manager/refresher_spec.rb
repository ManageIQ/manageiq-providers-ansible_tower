describe ManageIQ::Providers::AnsibleTower::AutomationManager::Refresher do
  ...
  let(:auth)                    { FactoryBot.create(:authentication, :userid => VcrSecrets.ansible_tower.user, :password => VcrSecrets.ansible_tower.password) }
  ...
  let(:provider) do
    ...
    FactoryBot.create(:provider_ansible_tower,
                       :zone       => zone,
                       :url        => VcrSecrets.ansible_tower.url,
